...
# Build the data
data <- read.csv('attendees.csv')
data <- data %>% count(Country) %>% data.frame %>% filter(Country != '') %>%
  arrange(desc(n))
data <- data %>% mutate(pc = n/sum(n)*100)
data <- data %>% mutate(label = ifelse(pc >= 1, as.character(Country), 'Others'))
